...
### 작업 체크리스트
- [ ] 시각적 디자인 개선
  - [ ] 좋아요/싫어요 버튼 스타일링 개선 (hover, active 상태)
  - [ ] CityCard 레이아웃 미세 조정 (간격, 패딩, 폰트 크기)
  - [ ] 필터 UI 스타일 개선 (Select, Checkbox 등)
  - [ ] 색상 일관성 확인 (Shadcn 테마 색상 사용)
- [ ] 반응형 디자인 최종 확인
  - [ ] 모바일 (375px): CityCard 1열 그리드, 필터 UI 스택 레이아웃
  - [ ] 태블릿 (768px): CityCard 2열 그리드, 필터 UI 적절한 배치
  - [ ] 데스크톱 (1920px): CityCard 3열 그리드, 넓은 필터 UI
- [ ] 접근성 개선
  - [ ] 좋아요/싫어요 버튼에 aria-label 추가
  - [ ] 필터 UI에 적절한 label 추가
  - [ ] 키보드 네비게이션 테스트
  - [ ] 색상 대비 확인 (WCAG AA 기준)
- [ ] 성능 최적화
  - [ ] 이미지 최적화 확인 (Next.js Image 컴포넌트 사용)
  - [ ] 불필요한 re-render 방지 (React.memo 필요 시)
...
### 검증 체크리스트
- [ ] `npm run build` 실행하여 프로덕션 빌드 성공하는지 확인
- [ ] 빌드 경고 메시지 확인 및 해결
- [ ] Lighthouse 점수 확인 (Performance, Accessibility, Best Practices, SEO)
  - [ ] Performance > 90
  - [ ] Accessibility > 90
- [ ] 전체 사용자 플로우 테스트
  - [ ] 홈페이지 접속 → 필터 선택 → 도시 확인 → 좋아요/싫어요 클릭
  - [ ] 모바일 디바이스에서 동일한 플로우 테스트
- [ ] 브라우저 호환성 테스트 (Chrome, Firefox, Safari)
- [ ] 콘솔 에러/경고가 없는지 최종 확인
- [ ] TypeScript 타입 에러가 없는지 확인 (`npx tsc --noEmit`)
- [ ] ESLint 경고가 없는지 확인 (`npm run lint`)
- [ ] CLAUDE.md 파일 업데이트
  - [ ] 새로운 데이터 구조 문서화
  - [ ] 필터 시스템 설명 추가
  - [ ] 좋아요/싫어요 기능 설명 추가
...
- [ ] **Phase 1**: 타입과 상수부터 정의 (데이터 구조의 기반)
- [ ] **Phase 2**: 불필요한 페이지와 네비게이션 정리 (프로젝트 구조 단순화)
- [ ] **Phase 3**: 불필요한 섹션 제거 (UI 정리)
- [ ] **Phase 4**: CityCard 재설계 및 도시 리스트 섹션 변경 (핵심 UI 구현)
- [ ] **Phase 5**: 가짜 데이터 업데이트 및 필터 UI 준비 (데이터 정비)
- [ ] **Phase 6**: 필터링 로직 구현 및 상태 관리 (기능 완성)
- [ ] **Phase 7**: UI 개선 및 최종 검증 (품질 보증)
...
- [ ] 이전 Phase가 완료되어야 다음 Phase 진행 가능
- [ ] 데이터베이스는 사용하지 않고 가짜 데이터로 진행
- [ ] 모든 기능은 클라이언트 사이드에서 동작 (로컬 상태 관리)
